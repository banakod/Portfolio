...
const highlights = [
  { icon: <FaCode />,             title: "Full-stack builds", text: "React interfaces connected to Node, Express, MongoDB, and clean API flows." },
  { icon: <FaWandMagicSparkles />,title: "Animated UI",       text: "Motion, transitions, and detail-focused layouts that keep the site alive." },
  { icon: <FaShieldAlt />,        title: "Security mindset",  text: "Practical attention to validation, backend boundaries, and safer data handling." },
];
...
const About = () => (
  <section id="about" className="py-24">
    <div className="section-shell">
      ...
      <motion.h2 className="section-title mb-7"
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {highlights.map((item, i) => (
            ...
            >
              ...
              <h3 className="text-lg font-black" style={{ color: "var(--text-primary)" }}>{item.title}</h3>
              <p className="mt-3 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>{item.text}</p>
            </motion.article>
          ))}
        </div>
      </div>
